...
<?php
  ...
  $fetch_all = mysqli_query($conn, "SELECT * FROM answers WHERE alumni_ID=$id");
  $get_row = mysqli_fetch_array($fetch_all);
?>
                  ...
                  <form action="process_save.php" method="POST" onsubmit="return onSubmit();">
                    ...
                    <div id="form-step-1" class="content p-4" role="tabpanel">
                      ...
                      <div class="form-group mb-3">
                        <?php 
                          $g_12 = $get_row['r_12'];
                          $twelve = mysqli_query($conn, "SELECT * FROM question WHERE quest_ID=$g_12");
                        ...
                        <table id="example1" class="table table-bordered table-sm table-hover">
                            ...
                            <tbody>
                                ...
                                mysqli_data_seek($fetch_all, 0);

                                while ($get_row2 = mysqli_fetch_assoc($fetch_all)) {
                                    // Unserialize the arrays
                                    $degrees = explode(",", $get_row2['degree']);
                                    $universities = explode(",", $get_row2['university']);
                                    $years = explode(",", $get_row2['year']);
                                    $honors = explode(",", $get_row2['honor']);

                                    // Assuming all arrays have the same length (adjust if needed)
                                    $count = count($degrees);

                                    // Iterate over each set of values and create a new row
                                    for ($i = 0; $i < $count; $i++) {
                                ?>
                                        <tr>
                                            <td><?php echo isset($degrees[$i]) ? trim($degrees[$i]) : ''; ?></td>
                                            <td><?php echo isset($universities[$i]) ? trim($universities[$i]) : ''; ?></td>
                                            <td><?php echo isset($years[$i]) ? trim($years[$i]) : ''; ?></td>
                                            <td><?php echo isset($honors[$i]) ? trim($honors[$i]) : ''; ?></td>
                                        </tr>
                                <?php 
                                    } // End inner loop
                                } // End outer loop
                     ...
                     <div class="form-group">
                        ...
                        <?php 
                          $g_13 = $get_row['r_13'];
                          $thirteen = mysqli_query($conn, "SELECT * FROM question WHERE quest_ID=$g_13");
                          $r_13 = mysqli_fetch_array($thirteen);
                        ?>
                        <label for=""><?= $r_13['question'] ?></label>
                        <table id="example22" class="table table-bordered table-sm table-hover">
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                ...
                                while ($get_row3 = mysqli_fetch_assoc($fetch_all)) {
                                    // Unserialize the arrays
                                    $examinations = explode(",", $get_row3['examination']);
                                    $dates_taken = explode(",", $get_row3['date_taken']);
                                    $ratings = explode(",", $get_row3['rating']);
                                ...
                                ?>
                                        <tr>
                                            <td><?php echo isset($examinations[$i]) ? trim($examinations[$i]) : ''; ?></td>
                                            <td><?php echo isset($dates_taken[$i]) ? trim($dates_taken[$i]) : ''; ?></td>
                                            <td><?php echo isset($ratings[$i]) ? trim($ratings[$i]) : ''; ?></td>
                                        </tr>
                                <?php 
                                    } // End inner loop
                                } // End outer loop
